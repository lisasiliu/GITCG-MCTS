...
from env_mini_random import GITCGRandomMiniGymEnv
import numpy as np
...
def play_one_game(env, seed=None, verbose=False):
    ...
    while not all(env.terminations.values()) and steps < safety_step_cap:
        ...
        dice_before = dice(agent)[:]   # copy list
        ...
        assert all(d >= 0 for d in dice(agent)), f"Negative dice after action {action_word} by agent {agent}"
        ...
        if verbose and should_print:
            ...
            line = _fmt_step_line(
                ...
                hp_opp=kaeya(opp)["hp"],
                dice_before=sum(d != 0 for d in dice_before),
                dice_after=sum(d != 0 for d in dice(agent)), # may show reset; see note below
...
if __name__ == "__main__":
    ...
    _ = play_one_game(GITCGRandomMiniGymEnv(), seed=123, verbose=True)
    print("Done.")
